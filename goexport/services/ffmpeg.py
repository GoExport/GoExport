...
class _PipeEncoder:
    ...
    def _failure(self) -> FFmpegError:
        ...
        tail = self._stderr.read().decode(errors="replace")
        return FFmpegError(
            f"FFmpeg failed (exit {self.process.returncode}): {' '.join(self.command)}\nstderr tail:\n{tail}"
        )
    ...
    def write_frame(self, data: bytes) -> None:
        ...
        try:
            ...
            while remaining:
                ...
                if not written:
                    raise FFmpegError("FFmpeg stdin stopped accepting data")
        ...
        try:
            if self.process.stdin is not None and not self.process.stdin.closed:
                try:
                    ...
                    # The return code and stderr below report the actual failure.
                    pass
            self.process.wait()
    ...
    def __init__(
        ...
        super().__init__(
            [
                ...
                "-",
                "-vf",
                f"crop={width}:{height}:0:0,pad=ceil(iw/2)*2:ceil(ih/2)*2",
                "-c:v",
                "libx264",
                "-pix_fmt",
                "yuv420p",
...
class FFmpegRawVideoEncoder(_PipeEncoder):
    def __init__(
        ...
        if output_width > width or output_height > height:
            raise ValueError(
                f"Capture {width}x{height} is smaller than requested output {output_width}x{output_height}"
            )
        super().__init__(
            [
                str(ffmpeg_path),
                ...
                "-",
                "-vf",
                f"crop={output_width}:{output_height}:0:0,pad=ceil(iw/2)*2:ceil(ih/2)*2",
                "-c:v",
                "libx264",
                "-pix_fmt",
                "yuv420p",
...
class FFmpegAudioEncoder:
    ...
    def encode(
        self, clips: list[AudioClip], output_file: Path, frame_count: int
    ) -> None:
        ...
        for i, clip in enumerate(clips):
            ...
            if clip.has_trim:
                trim_start = clip.trim_start_frame / self.fps
                trim_end = (
                    min(
                        clip.trim_end_frame,
                ...
                source = f"atrim=start={trim_start:.6f}:end={trim_end:.6f}"
            else:
                source = f"atrim=end={clip.duration_frames / self.fps:.6f}"
            filters.append(f"[{i}:a]{source},adelay={delay}|{delay}[a{i}]")
            inputs.append(f"[a{i}]")
        filters.append(
            "".join(inputs)
            + f"amix=inputs={len(clips)}:normalize=0,atrim=end={duration:.6f}"
        )
        subprocess.run(
            command
            + [
...
class FFmpegMuxer:
    ...
    def mux(self, video_file: Path, audio_file: Path | None, output_file: Path) -> None:
        ...
        if audio_file is None:
            # Keep the requested container valid and allow audio-based outro concat.
    ...
    def append_outro(
        ...
        if not main_video_file.is_file() or not outro_file.is_file():
            raise FileNotFoundError("Main video or outro does not exist")
        ...
        self._run(
            [
                ...
                str(outro_file),
                "-filter_complex",
                f"[0:v]fps={fps},setpts=N/({fps}*TB)[a];[1:v]scale={width}:{height}:force_original_aspect_ratio=decrease,pad={width}:{height}:(ow-iw)/2:(oh-ih)/2,fps={fps},setpts=N/({fps}*TB)[b];[a][0:a][b][1:a]concat=n=2:v=1:a=1[v][a]",
                "-map",
                "[v]",
                "-map",
                "[a]",
