...
from goexport.services.capture import VideoSelector, audio_padding_samples, audio_trim_samples, create_capturer, timestamp_ns
from goexport.services.ffmpeg import FFmpegMuxer, FFmpegRawAudioEncoder, FFmpegRawVideoEncoder
from goexport.services.flash import await_player_ready, await_started, await_stopped

logger = logging.getLogger(__name__)
_AUDIO = {"int8": ("s8", 1), "int16": ("s16le", 2), "int32": ("s32le", 4), "int64": ("s64le", 8), "uint8": ("u8", 1), "uint16": ("u16le", 2), "uint32": ("u32le", 4), "uint64": ("u64le", 8), "float32": ("f32le", 4), "float64": ("f64le", 8)}
...
class RecordingService:
    ...
    def _capture_streams(self, capturer, video_path, audio_path, started, stopped):
        import scap
        video = audio = None; selector = VideoSelector(config.FPS); video_origin = audio_origin = None; pending = []; boundary = None; self._audio_aligned = False
        try:
            while True:
                ...
                if not started.is_set(): continue
                now = timestamp_ns(frame.timestamp)
                if stopped.is_set() and boundary is None: boundary = now
                if boundary is not None and now > boundary: break
                if isinstance(frame, scap.VideoFrameInfo):
                    if frame.format not in {"bgra", "bgr0"}: raise RuntimeError(f"unsupported scap video format: {frame.format}")
                    if video is None: video = FFmpegRawVideoEncoder(config.FFMPEG_PATH, video_path, frame.width, frame.height, *self.args.resolution, config.FPS)
                    if video_origin is None: video_origin = now
                    for data in selector.add(now, frame.data.tobytes()): video.write_frame(data)
                    for item, stamp in pending: self._write_audio(item, stamp, video_origin, audio, boundary)
                    pending.clear()
                elif isinstance(frame, scap.AudioFrameInfo):
                    if frame.planar: raise RuntimeError("Planar scap audio is not supported")
                    spec = _AUDIO.get(frame.format)
                    if spec is None: raise RuntimeError(f"unsupported scap audio format: {frame.format}")
                    if audio is None:
                        audio = FFmpegRawAudioEncoder(config.FFMPEG_PATH, audio_path, frame.channels, frame.rate, spec[0]); self._sample_bytes = spec[1]
                    if audio_origin is None: audio_origin = now
                    if video_origin is None: pending.append((frame, now))
                    else: self._write_audio(frame, now, video_origin, audio, boundary)
                else: raise RuntimeError(f"unexpected scap frame type: {type(frame)!r}")
                if boundary is not None: break
        finally:
            try: capturer.stop()
            except Exception: logger.debug("capturer.stop failed during cleanup", exc_info=True)
            if video is not None:
                if boundary is not None:
                    for data in selector.finish(boundary): video.write_frame(data)
                video.close()
            if audio is not None: audio.close()
        if audio_origin is not None and video_origin is not None: logger.debug("Initial audio/video offset: %.3f ms", (audio_origin - video_origin) / 1_000_000)

    def _write_audio(self, frame, stamp, video_origin, encoder, boundary=None):
        data = frame.data.tobytes(); offset = stamp - video_origin; size = frame.channels * self._sample_bytes
        if not self._audio_aligned:
            if offset > 0: encoder.write_frame(b"\0" * (audio_padding_samples(offset, frame.rate) * size))
            elif offset < 0: data = data[min(len(data), audio_trim_samples(offset, frame.rate) * size):]
        ...
        if boundary is not None:
            ...
            allowed = max(0, min(frame.sample_count, (boundary - stamp) * frame.rate // 1_000_000_000))
            data = data[:allowed * size]
        if data: encoder.write_frame(data)

    def _prepare_browser(self):
        service = self._create_browser_service(); driver = service.create_driver(); driver.get(self.args.url); service.enter_fullscreen(driver); service.validate_screen_resolution(driver); service.assert_full_resolution(); service.enable_flash(driver); service.inject_dom(driver, config.TEMPLATE_HTML_PATH, self._build_replacements()); await_player_ready(driver); return service, driver
    @staticmethod
    ...
    def _record_playback(self, driver, video_path, audio_path):
        ...
        try:
            ...
            if watcher.is_alive(): raise TimeoutError("Playback did not stop within 30 seconds")
    ...
    def _finish_recording(self, output, video, audio):
        ...
        if audio.is_file(): muxer.mux(video, audio, output)
    ...
    def run(self):
        ...
        try:
            ...
            if complete: video.unlink(missing_ok=True); audio.unlink(missing_ok=True)
            elif video.exists() or audio.exists(): logger.error("Retaining capture diagnostics after failure: %s, %s", video, audio)
        return 0
