...
- Added more debugging for browsers
- Added a failure when you close Chromium while it's recording

## [1.2.2] - 2026-03-22

### Added

- Added a label to both input fields; "Video ID" and "User ID" to make it more clear what they are for.
...
- Added a flag to skip the resolution check (useful for multi-monitor setups or virtual displays) you can use the argument:
  - `--skip-resolution-check` : Skips the check that ensures the selected resolution does not exceed the monitor's resolution.
- Added a flag to select which monitor to use for resolution checking (useful for multi-monitor setups) you can use the argument:
  - `--monitor-index 0` : Replace `0` with the index of the monitor you want to use for resolution checking (0-based index).

## [1.1.6] - 2025-12-30

### Fixed

- Added a new flag to change the audio capture device used by FFmpeg on Linux:
  - `--pulse-audio "alsa_output.pci-0000_00_1b.0.analog-stereo.monitor"` : Replace the value with your desired PulseAudio source.
